...
class WordSelect():
    # dict of (min_len, max_len):[func_list] pairs
    function_registry = defaultdict(list)

    def __call__(self, min_len, max_len):
        """
        Decorator for word-processing functions. Apply to functions to select
    ...
    def process_words(self):
        ...
        list of functions. If so, call all the functions for the word and print
        the result.

        :return: None. Note: could return list of words,
        ...
        with open('../../data/words.txt') as words_in:
            for raw_word in words_in:
                ...
                word_len = len(word)
                items = self.function_registry.items()
                for (min_len, max_len), word_functions in items:
